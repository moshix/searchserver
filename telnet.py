...
class TelnetServer:
    ...
    def handle_client(self, client_socket, client_address):
        ...
        try:
            ...
            # Send welcome and help messages to the client
            welcome_message = f"{COLOR_GREEN}Welcome to the Telnet server!{COLOR_RESET}\r\n"
            help_message = self.show_help()

            client_socket.sendall(welcome_message.encode('utf-8'))
            client_socket.sendall(help_message.encode('utf-8') + b'\r\n')
            ...
            while True:
                try:
                    ...
                    clear_lines = "\n\n"
                    client_socket.sendall((clear_lines + response).encode('utf-8') + b'\r\n')
    ...
    def show_help(self):
        ...
        help_text = (
            ...
            f"{COLOR_BLUE}/help{COLOR_RESET} - Show this help message\r\n"
            f"{COLOR_BLUE}/search <keyword>{COLOR_RESET} - Search files in the FILES/ directory for a keyword\r\n"
            f"{COLOR_BLUE}/videosearch <keyword>{COLOR_RESET} - Search for lines containing the keyword in videos.txt\r\n"
            f"{COLOR_BLUE}/logoff{COLOR_RESET} - Log off from the server\r\n"
            f"{COLOR_BLUE}/stats{COLOR_RESET} - Show server statistics\r\n"
            f"{COLOR_BLUE}/uptime{COLOR_RESET} - Show server uptime and start time"
        )
        return help_text
    ...
    def get_uptime(self):
        ...
        return (
            f"{COLOR_GREEN}Server Uptime:{COLOR_RESET}\r\n"
    ...
    def get_stats(self):
        ...
        with self.lock:
            ...
            stats_text = (
                f"{COLOR_BLUE}Server Statistics:{COLOR_RESET}\r\n"
